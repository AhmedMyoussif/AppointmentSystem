...
public sealed class GetCustomerAppointmentsQueryHandler : IRequestHandler<GetCustomerAppointmentsQuery, Result<List<AppointmentDto>>>
{
    ...
    public GetCustomerAppointmentsQueryHandler(IAppDbContext context)
    {
        _context = context;
    }

    public async Task<Result<List<AppointmentDto>>> Handle(GetCustomerAppointmentsQuery request, CancellationToken ct)
    {
